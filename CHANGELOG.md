...
- **嵌入思维导图自动重新加载**：完善文档刷新后的思维导图加载机制
  - 自动检测：每10秒自动检测并加载未初始化的嵌入思维导图块
  - 手动重新加载：顶栏菜单新增"检测并加载嵌入思维导图"选项
  - 单个重新加载：每个嵌入思维导图界面添加"重新加载"按钮
  - 强制重新加载：支持完全清理旧实例并重新创建
...
- **修复文档刷新后嵌入思维导图不加载的问题**
  - 实现自动检测机制，文档刷新后自动重新加载
  - 添加强制重新加载功能，清理旧实例避免内存泄漏

### 🚀 性能优化

- 日志输出优化：内存占用显著降低，控制台更加整洁
- 适合长时间使用，不会因日志累积导致性能下降
- 嵌入思维导图重新加载时完全清理旧实例，避免内存泄漏
