 ...
 * API 文档�?[API_zh_CN.md](https://github.com/siyuan-note/siyuan/blob/master/API_zh_CN.md)
 */
...
export async function getWorkspaces(): Promise<IResGetWorkspaces> {
...
export async function moveDocToParent(docId: DocumentId, targetDocId: DocumentId | null, targetNotebook: NotebookId): Promise<any> {
    const data = {
        fromIDs: [docId],
        toID: targetDocId || targetNotebook
    };
...
/**
 * 获取文档的反向链接
 * @param blockId 文档块 ID
 * @returns 反向链接列表
 */
export async function getBacklink2(blockId: BlockId): Promise<IResGetBacklink2> {
    const data = {
        id: blockId
    };
 ...
 * @returns 引用ID列表
 */
export async function getRefIDs(blockId: BlockId): Promise<{ refIDs: BlockId[], refTexts: string[] }> {
    const data = {
        id: blockId
    };
    const url = '/api/ref/getRefIDs';
    return request(url, data);
}